...
public:
    ...
    void SetBlendConstants( const float in_blendConstants[4] );

    void SetDepthBounds( const float in_minDepthBounds, const float in_maxDepthBounds );

    void SetStencilCompareMask( const VkStencilFaceFlags in_faceMask, const uint32_t in_compareMask );

    void SetStencilWriteMask( const VkStencilFaceFlags in_faceMask, const uint32_t in_writeMask );

    void SetStencilReference( const VkStencilFaceFlags in_faceMask, const uint32_t in_reference );
    ...
    void BindIndexBuffer(   const VkBuffer in_buffer, 
                            ...
                            const VkIndexType in_indexType );
    ...
    void Dispatch(  const uint32_t in_groupCountX, 
                    ...
                    const uint32_t in_groupCountZ );

    void DispatchIndirect( const VkBuffer in_buffer, const VkDeviceSize in_offset );

    void CopyBuffer(    const VkBuffer in_srcBuffer, 
                        const VkBuffer in_dstBuffer, 
                        const uint32_t in_regionCount, 
                        const VkBufferCopy* in_regions );
    ...
    void CopyBufferToImage( const VkBuffer in_srcBuffer, 
                            ...
                            const VkImageLayout in_dstImageLayout, 
                            const uint32_t in_regionCount, 
                            const VkBufferImageCopy* in_regions );
    ...
    void  ClearColorImage(  const VkImage in_image, 
                            ...
                            const VkImageSubresourceRange* in_ranges );
